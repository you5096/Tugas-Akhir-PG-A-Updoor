...
Trap::Trap(float swtile, float shtile, float xtile, float ytile) {
	this->swtile = swtile;
	this->shtile = shtile;
	this->xtile = xtile;
	this->ytile = ytile;
...
void Trap::BuildTrapSprite()
{
	...
	// Load, create texture 
	int width, height;
	unsigned char* image = SOIL_load_image("trap.png", &width, &height, 0, SOIL_LOAD_RGBA);
...
void Trap::DrawTrapSprite(float width, float height, float x, float y) {
	glEnable(GL_BLEND);
